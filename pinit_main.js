	...
	var e = a[d.k] = {
		...
		f: function() {
			return {
				...
				structure: {
					...
					embedPin: function(a, b) {
						...
						if(b.width || (b.width = "small"), a.data && a.data[0]) {
								...
								if(g.story_pin_data.page_count) {
									...
									for(c = 0; c < g.story_pin_data.pages.length; c += 1) {
										if(h = g.story_pin_data.pages[c], f = {
												addClass: "page "
											}, (h.style || {}).background_color && (f.backgroundColor = h.style.background_color), canHazPage = !1, "0.1.0" === q.version && (canHazPage = !0, f.blocks = {
												0: {
													addClass: "block",
													height: "100%",
													width: "100%"
											...
											}), c || (f.blocks[0].image.addClass = "coverMe")), "0.3.0" === q.version)
											if(canHazPage = !0, f.blocks = {
													0: {
														addClass: "block",
														height: "100%",
														width: "100%"
											...
											var u = h.video.video_list.V_HLSV3_MOBILE,
												v = u.url.replace(/\/hls\//, "/720p/").replace(/\.m3u8/, ".mp4");
											u.url ? (f.blocks[0].container = {
												addClass: "video",
												video: {
													height: "100%",
													width: "100%",
											...
											}, f.addClass = f.addClass + "hazVideo ") : u.thumbnail && (f.blocks[0].image = {
												height: "100%",
												width: "100%",
												backgroundImage: u.thumbnail.originals.url.replace(/\/originals\//, "/" + imgWidth + "/")
											})
										}
										if("0.4.0" === q.version)
											if(canHazPage = !0, f.blocks = {
											...
											var u = h.video.video_list.V_HLSV3_MOBILE,
												v = u.url.replace(/\/v2\/hls\//, "/720p/").replace(/_mobile\.m3u8/, ".mp4");
											u.url ? (f.blocks[0].video = {
												height: "100%",
												width: "100%",
												mp4: v
											}, f.addClass = f.addClass + "hazVideo ") : u.thumbnail && (f.blocks[0].image = {
										...
										if("0.7.0" === q.version && h.image_adjusted && h.image_adjusted.images && (canHazPage = !0, f.blocks = {
												0: {
													addClass: "block",
													height: "100%",
													width: "100%",
											...
											}, c || (f.blocks[0].image.addClass = "coverMe")), ("0.8.0" === q.version || "0.9.0" === q.version || "0.10.0" === q.version) && (canHazPage = !0, h.blocks)) {
											f.blocks = {};
											var w = 0;
											for(d = 0; d < h.blocks.length; d += 1) {
												...
												if(y) {
													...
													if(z.font) {
														var B = z.font.key;
														e.v.fonts[B] || (e.v.fonts[B] = {
															name: z.font.name,
															url: z.font.url
														}, e.f.loadFont(e.v.fonts[B])), A.fontFamily = z.font.name
													}
													if("story_pin_heading_block" === x.type) {
														x.backgroundColor = y.highlight_color;
														var C = "top",
															D = "left";
														z.alignment && 1 === z.alignment && (D = "center"), z.vertical_alignment && (1 === z.vertical_alignment && (C = "middle"), 2 === z.vertical_alignment && (C = "bottom")), A.container = {
															addClass: D + " " + C,
															paragraph: {
																backgroundColor: z.highlight_color || "",
																color: z.hex_color || "",
																text: x.text.replace(/(\n+)/g, "<br>")
															}
														}
													}
													if("story_pin_image_block" === x.type && (A.height = y.height + "%", A.width = y.width + "%", A.image = {
															addClass: "containMe",
															height: "100%",
															width: "100%",
															backgroundImage: x.image.images.originals.url.replace(/\/originals\//, "/" + imgWidth + "/"),
															backgroundColor: x.image.dominant_color || "#888"
														}, "100" !== y.height && (A.image.addClass = "coverMe")), "story_pin_video_block" === x.type && ((x.video || {}).video_list || {}).V_720P && ((x.video || {}).video_list || {}).V_HLSV4) {
														var E = x.video.video_list.V_720P,
															F = x.video.video_list.V_HLSV4,
															G = x.video.video_list.V_720P.thumbnail || x.video.video_list.V_HLSV4.thumbnail;
														E.url && F.url && (A.container = {
															addClass: "video",
															video: {
																mp4: E.url,
																m3u8: F.url,
													...
													if("story_pin_paragraph_block" === x.type) {
														x.backgroundColor = y.highlight_color;
														var C = "top",
															D = "left";
														z.alignment && 1 === z.alignment && (D = "center"), z.vertical_alignment && (1 === z.vertical_alignment && (C = "middle"), 2 === z.vertical_alignment && (C = "bottom")), A.container = {
															...
															paragraph: {
																backgroundColor: z.highlight_color || "",
																color: z.hex_color || "",
																fontSize: z.font_size * j + "px",
																lineHeight: lineHeight,
																text: x.text.replace(/(\n+)/g, "<br>")
															}
														}, z.font && z.font.min_size && z.font.min_size < z.font_size - 1 && (A.left = "2%", A.width = "96%")
													}
													f.blocks["" + w] = A, w += 1
												}
											}
							...
							var H, I;
							g.native_creator ? (I = ((g.story_pin_data || {}).metadata || {}).pin_title || n((g.story_pin_data || {}).pages) || g.native_creator.about, I = I.replace(/\s/g, " ").replace(/  +/g, " "), H = {
								"native": !0,
								...
								name: m
							}, (g.attribution || {}).author_name && (H.credit = e.a.strings[b.lang].by.replace(/%s/, g.attribution.author_name))) : H = {
							...
							}, i.footer.container.deets.bottomline.text ? H["native"] && (i.footer.addClass = "native") : i.footer.addClass = "uno";
							var J = e.f.buildOne(i);
							return e.f.changeClass(J, {
								fresh: !0
							}), e.f.listen(J, "mouseover", function() {
								e.f.changeClass(J, {
									fresh: !1
								})
							}), e.f.set(J, "data-pin-current", "0"), J
						}
					}
				},
				getLegacy: {
				...
				getLang: function(a) {
					var b, c, d = e.a.lang;
					return "string" !== typeof a && (a = d), b = a.toLowerCase(), b = b.replace(/[^a-z0-9]/g, " "), b = b.replace(/^\s+|\s+$/g, ""), b = b.replace(/\s+/g, " "), b = b.split(" "), b.length && e.a.strings[b[0]] && (d = b[0], b.length && (c = d + "-" + b[b.length - 1], e.a.strings[c] && (d = c))), d
				},
